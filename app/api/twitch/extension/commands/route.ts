...
import { refundViewerSparks, spendViewerSparks } from "../../../../../lib/backend/viewer-wallet";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const viewerParameters = body.viewerParameters && typeof body.viewerParameters === "object"
      ...
      : {};
    const spendMarker = crypto.randomUUID();
    await spendViewerSparks(claims, effect.defaultCreditCost, spendMarker);
    let command;
    try {
      command = await enqueueViewerCommand(claims, effect, viewerParameters, {
        monetization: {
          source: "sparks",
      ...
      await refundViewerSparks(claims, effect.defaultCreditCost, spendMarker);
      throw error;
    }
